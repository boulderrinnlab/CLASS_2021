...
#### we can get values using runValue()
#### let's say we want the values in the seqnames
runValue(seqnames(gencode_gr))[1:25]
runValue(seqnames(gencode_gr))[1:50]
#### same result since there are only 25 levels for seqnames to grab.


#### Let's get the length of values in the ranges with runLength.
runLength(seqnames(gencode_gr))
...
mrna_df <- filter(gencode_gr_length, gene_type == "protein_coding")
hist(mrna_df$width, breaks = 2)
...
lncrna_df <- filter(gencode_gr_length, gene_type == "lncRNA")
hist(lncrna_df$width, breaks = 2)
...
## So which gene is it !! There are lots of ways to find out.
summary(longest_gene_index)
## This gives us the same as the values table in environment it's about 44Kb long
...
# Now lets' select features from gencode and make a data_frame of useful info
```{r}
gencode_gr <- rtracklayer::import("data/gencode.v32.annotation.gtf")
#### Here we see a "pipe" that is going to do a lot to our gencod_gr GRranges.
#### First read in gencode_gr -- we will deal with what is happening here
#### later in the GRanges .RMD

gencode_gm <- gencode_gr %>%
  #### converting the GRanges to data.frame
  as.data.frame() %>%
  #### applying filter step to
  filter(type == "gene_name") %>%
  dplyr::select(seqnames, start, end, width, gene_type,
  # Here dplyr select will grab the seqnames, start, end, width adn genetype for all   # genes
                gene_id, gene_name, transcript) 
names(gencode_gm) <- c("chromosome", "start", "end", "width",
  # names is adding the names to the columns of features we outlined above
                       "gene_id", "gene_name", "transcript")

# Now we hav a handy data frame of the start and stop of all genes. We can write it 
# out with write_csv
...
## TODO discuss file types this maybe a good error and then fix
findOverlaps(pol2_chip, gencode_gr@elementMetadata$gene_id)
...
oh_no <- gencode_gr[which(seqnames(gencode_gr == "gene_id"))]
