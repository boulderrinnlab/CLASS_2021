...
# 
ggdendro::ggdendrogram(bin_hier_lncrna, rotate = TRUE,  size = 70)
# 
ggsave("chipseq/figures/lncrna_hclust_binary_dist.pdf", height = 70, width = 70)
# 
...
high_binders <- promoter_peak_occurence[,colSums(promoter_peak_occurence) > 350]
...
plot(zf_hclust)
pdf("figures/zf_hclust.pdf", height = 12, width = 70)

#Filtering for ZNF343

number_of_dbp <- colSums(promoter_peak_occurence)
high_binding_promoters <- number_of_dbp >350

high_binding_ppo <- promoter_peak_occurence[,high_binding_promoters]

znf343_promoters <- high_binding_ppo[rownames(high_binding_ppo) == 'ZNF343',]
znf343_promoters_df <- data.frame('gene_id' = colnames(znf343_promoters), 'number_of_dbp' = colSums(znf343_promoters))
ZNF343_promoters <- znf343_promoters[colSums(znf343_promoters) == 1]
ZNF343_promoters <- data.frame(t(znf343_promoters))

length(znf343_promoters) #1514

znf343_promoters_id <- as.character(znf343_promoters$gene_name)
...
merge_df <- merge(znf343_promoters_df, peak_occurence_df_new, by = "gene_id")

Nina_ATF5 <- read.csv('/scratch/Shares/rinnclass/Nina/CLASS_2021/analysis/promoter_ATF5_bound_df.csv')

Nina_ZNF34 <- read.csv('/scratch/Shares/rinnclass/Nina/CLASS_2021/analysis/promoter_ZNF34_bound_df.csv')

ATF5_ZNF34_ZNF343 <- merge(merge_df, Nina_ZNF34, by = 'gene_id')
#ATF5 is not complete here, may merge with Victor?

write_csv(ATF5_ZNF34_ZNF343, '/scratch/Shares/rinnclass/Olivia/CLASS_2021/analysis/clustering_promoters_df.csv')
```

```{r}
#Nina's code for comparison
...
#Hmm this also doesn't work. ColSums doesn't cooperate.
```
