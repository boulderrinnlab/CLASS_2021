...
First we need to se up all the details of the run. Nextflow will go to the 
...
We will use SBATCH to submit batch instructions to slurm. There are more complicated
ways to use SBATCH for heterogenous job instructions. Here we just want to have
SBATCH send all the needed inforamtion. 

In otherwords: When the job allocation is finally granted for the batch script,
...
#### There are X nodes on Fiji, and N CPU 
...
-c telling the c-shell to configure nextflow with the config file (described above)
...
*******************************************
Step 3:Running and Checking on slurm jobs
*******************************************

First thing we want to do is "run" the file. Since we set this up in SBATCH we
simply type:
...
--- what jobs are you running? You often want to see what job is running how long
it is taking and other aspects. To that end we can use 'squeue' to see what is runnign

```
squeue -u identikey
```
the -u flag is for user.
examine the information provided and google what it means.
...
Here we are using some bash to grab all the jobs running. They will typically have
a common number (591 in example above -- would have aloso worked with just 5).

Grep finds all your job id matches, then awk prints them, xargs is then going 
to perform scancel on the output of awk, the -n is telling xargs how many arguments
will follow. In this case just 1 -- scancel.
