...
ov2 <- reduce(c(ir1,ir2))
plotRanges(ov2)
# reduce does the same thing where we saw above it reduced the overlapping ranges
# into one start and stop covering both ranges. The same thing happened here.
# it merged the two overlapping peaks and same for the non overlapping interval.
# it took the largest window in the ranges and made two ranges one.
...
## ??? TODO how to paste in new values for Chry
GRanges(ranges = Iranges(start = c(20, 40, 60), levels(chrY)
                         
                         #or index directly?
                         
                        # sapply(levels(gr(=chrY"), IRanges(start = c(20, 40, 60), end = c(37, 47, 67)))
# let's add genome:
