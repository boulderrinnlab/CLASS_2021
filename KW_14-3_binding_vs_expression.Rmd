...
#samplesheet written out into the rinnclass data folder by Michael
```
...
make TSS profile heatmap for DBPs bound at reservoirs (like analysis 03)
```{r}

#make a column to mark conserved reservoirs in both cell lines
promoter_features_df$conserved_reservoir <- 
  as.numeric(promoter_features_df$hepg2_reservoir == 1 & 
             promoter_features_df$k562_conservative_reservoir == 1)

#filter to only have df with conserved reservoirs
conserved_reservoir_df <- filter( promoter_features_df, conserved_reservoir == 1)

#load in gencode gr annotations and peak maps
...
#KRTAP10-6 reservoir
KRTAP_promoter <- all_promoters_gr[all_promoters_gr$gene_name == "KRTAP10-6"]
KRTAP_promoter_matrix <- make_promoter_binding_matrix(consensus_peaks, KRTAP_promoter)
plot_promoter_peak_matrix(KRTAP_promoter_matrix, gene_name = "KRTAP10-6", save_pdf = TRUE)
