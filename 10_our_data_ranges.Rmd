...
fl <- list.files("/Users/niri2724/CLASS/data/consensus_peaks", full.names=TRUE)


consensus_peaks <- lapply(fl, rtracklayer::import, format = "bed")

table(consensus_peaks[[42]]$name) %>%
  head()
...
#hist(num_peaks_df$num_peaks, breaks = 1000)

hist(num_peaks_df$num_peaks, breaks = 1000, xlim = c(0, 2000))

#new line just added to keep it more save 
#dbps_with_sufficient_peaks <- num_peaks_df$num_peaks_df$num_peaks > num_peaks_threshold]

filtered_consensus_peaks <- consensus_peaks[num_peaks_df$num_peaks > num_peaks_threshold]

# or filtered_consensus_peaks <- consensus_peaks[dbps_with_sufficient_peaks]
...
library(janitor)
human_tfs <- readxl::read_excel("/Users/niri2724/CLASS/data/mmc2.xlsx",
                                sheet = 2, skip = 1)
