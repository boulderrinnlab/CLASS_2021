...
hist(num_peaks_df$num_peaks, breaks = 1000, xlim = c(0, 2000))
filtered_consensus_peaks <- consensus_peaks[num_peaks_df$num_peaks > 500]
#cuttoff at the 8th percentile to remove DBPs with very small number of peaks
...
#this needs the folders to already be there
#can count the number of output files in terminal using list (ls | wc -l)
```
...
library(janitor) #janitor strips away extra excel formatting to help convert to df
human_tfs <- readxl::read_excel("/scratch/Shares/rinnclass/data/mmc2.xlsx",sheet = 2, skip = 1)
names(human_tfs)[4] <- "is_tf"

# Checking to see how many overlaps there are.
length(which(tolower(num_peaks_df$dbp) %in% tolower(human_tfs$Name)))
#425 of our 446 DBPs are in the TFs list we just pulled from another paper (link above)

length(which(is.na(num_peaks_df$ensembl_id)))

# TODO: Manually match names -- doublecheck
# Subset to just the columns we need. Overrides the human_tfs df to only include the data we want
...
#add if it's a TF on the other list to our running num_peaks_df that contains most of our wanted information that we keep adding on to 
#(since human_tfs is shorter the ones that aren't in the TFs list will have null (NA) for DBPs that aren't in the TFs list)
