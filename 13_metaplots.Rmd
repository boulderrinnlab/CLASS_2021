...
peak_coverage <- (consensus_peaks[["ZNF34"]])
...
ggplot(metaplot_df, aes(x = x, y = dens)) + 
  geom_vline(xintercept = 0, lty = 2) + 
  geom_line(size = 1.5) + 
  ggtitle("ZNF34 Promoter Metaplot") + 
  scale_x_continuous(breaks = c(-3000, 0, 3000),
                     labels = c("-3kb", "TSS", "+3kb"),
                     name = "") + 
  ylab("Peak frequency")
ggsave("ZNF34_promoter_metaplot.pdf")
```
...
clusters <- cutree(metaplot_hclust, h = 75.0)
table(clusters)

clus1 <- mm_scaled[clusters == 2,]
dim(clus1)
...
?Heatmap
pdf("figures/tss_profile_heatmap_25.pdf", height = 35, width = 10)
