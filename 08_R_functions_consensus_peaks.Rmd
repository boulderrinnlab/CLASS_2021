...
## entry.
  ...
  # then the for loop will move onto the next unique_tf and merge those peak files.
    ...
    # setting up a second for loop, i is reading in the files and j is going to
    # merge peaks of replicates after the input from i.
      
      peak_list <- c(peak_list, read_peaks(tf_files[j]))
    }

## As the last step in the j for loop we call a function we made 'read_peaks'! So now we get
## to see how handy it is to make a function call in a function and not have to 
## type all the code out. 
...
## in this for loop we will filter out non-cannonical chromosome names (first)
...
## seqnames (?seqnames) (? which) that are %in% (handy r function!) our cannonical chr list.
