...
create_consensus_peaks <- function(broadpeakfilepath = "/scratch/Shares/rinnclass/data/peaks") {
  ...
  fl <- list.files(broadpeakfilepath, 
                   full.names = TRUE)
  fl <- fl[1:20]
  tf_name <- sapply(fl, function(x){ #read and find tfs in /data/peaks directory
 
    file_name <- str_extract(x, "[\\w-]+\\.broadPeak")
    return(str_extract(file_name, "^[^_]+(?=_)"))
  })
  
  #remove tf_name redunduncy
  unique_tf <- unique(tf_name)
  
  #find consensus_peaks of each unique_tf
  consensus_peaks <- list()

  for (i in 1:length(unique_tf)) {
    dbp <- unique_tf[i]
    cat("Now I'm running", dbp, "\n") #print current dbp[i]
    tf_files <- fl[grep(dbp, fl)] #subset only dbp[i] of fl
