...
First, read in the data from the sample sheet.
...
First, we will read in the gene annotations and let Salmon map transcripts to these different genes. Then we write out a new data frame with the gene names and ID for every gene.
...
Now we are going to slowly build the data frame that shows the gene name, and its expression in various subcellular fractions.
Here, we are taking the raw counts from the RNAseq nextflow and creating a matrix with those counts to be plugged into the final data frame later.
```{r}
...
Here we are combining the matrices that we have made and ordering the sheet to be used in DESeq analysis.
...
We will start plugging information into the results data frame, where we will have a column with each differential expression to total, for four total columns.
...
Now read in the rest of the fractions and plug them into the samplesheet.
...
Do the same Salmon read in as above for each of the new fraction comparissons. 
...
Now we have plugged in all of the other columns, so we can start analyzing these as above.
##Something in here is messed up still :(
```{r}
