...
Now we can start looking at the results from the DESeq analysis: we will make a data frame with the results from each fraction:
...
Now we can add in a new column that will tell us if the promoter acts as a resevoir in both cell lines, and allow us to index in these genes specifically. 

```{r}

?filter
shared_conservative_resevoirs_df <- filter(promoter_features_df, promoter_features_df$shared_conserved_resevoirs == 1)
write_csv(shared_conservative_resevoirs_df, "/scratch/Shares/rinnclass/James/CLASS_2021/analysis/shared_conservative_resevoirs.csv")
```
