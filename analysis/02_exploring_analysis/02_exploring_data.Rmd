...
matrix_colnames <- colnames(occurence_matrix)
matrix_rownames1 <- rownames(occurence_matrix[1])
matrix_column1 <- occurence_matrix[1]
matrix_column2 <- occurence_matrix[2]
...
#filter mRNA_lncrna_promoters to be just promoters with lots of DBPs bound (second_peak_gene_ids)
promoter_peak_occurence <- count_peaks_per_feature(second_peak_gene_ids_Grange, filtered_consensus_peaks, type = "occurrence")
#IDs needs to be Granges object


## attempt 4 works!
#matrix of only promters in the second hump and occurence matrix for each DBP
super_promoter <- occurence_matrix[,colSums(occurence_matrix)>350]
#350 cuttoff looks at promoters past the second peak
...
#re-order the merged table (defaults to new info on the far right)
...
super_promoter_features <- super_promoter_features %>%
  mutate(percent_super_prom_ov = num_super_promoter_overlaps/peaks_overlapping_promoters * 100, 
         ...
         zincfinger = ifelse(grepl(" ZF", dbd), "ZF", 
                             "Other"))
...
ZF_matrix <- C2H2_ZF_names %in% lncrna_mrna_promoter_peak_occurence_matrix
