...
po_K562_df <- read.csv(file = '/scratch/Shares/rinnclass/data/2020_peak_occurence_df.csv')
po_HEPG2_df <- read.csv(file = '/scratch/Shares/rinnclass/tardigrades/CLASS_2021/analysis/01_global_peak_properties/results/peak_occurence_dataframe.csv')
...
po_K562_df <- po_K562_df %>%
  mutate(num_dbp_norm_K562 = number_of_dbp/length(po_K562_df$dbp))

po_HEPG2_df <- po_HEPG2_df %>%
  mutate(num_dbp_norm_HEPG2 = number_of_dbp/nrow(po_HEPG2_df))
```
...
high_binder_df <- po_HEPG2_df[po_HEPG2_df$number_of_dbp >= 200&po_HEPG2_df$number_of_dbp <= 350,]
high_K562_df <- po_K562_df[po_K562_df$gene_id %in% high_binder_df$gene_id,]
...
reservoir_K562 <- po_K562_df %>% filter(po_K562_df$reservoir == 1)
reservoir_HEPG2_df <- po_HEPG2_df[po_HEPG2_df$gene_id %in% reservoir_K562$gene_id,]

#And finally, we'll find which reservoir promoters are also high binders.
filtered_df$num_dbp_norm_K562 <- reservoir_K562$num_dbp_norm_K562
reservoir_high_df <- filtered_df[filtered_df$gene_id %in% high_binder_df$gene_id,]
```
