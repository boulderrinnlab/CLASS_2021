...
##14-2 Part 2: using all the fractionation RNA-seq samples
load all libraries/functions from first chunk
load gtf and spreadsheet 
(rung the first 3 chunks in total)
...
use a for loop for making data frame with all of results 
(master res and res_shrunken for each sample and merge all dfs)
simplify to df that just has fold changes and sample comparison title and gene name

```{r}
## making a heatmap comparing the difference in RNA present in each cellular fraction vs total

#set up loop variables
conditions <- resultsNames(dds)
conditions <- conditions[2:5]
master_res_df <- data.frame("gene_id" = character(), 
                              "baseMean" = numeric(), 
...
master_res_shrunken_df <- data.frame("gene_id" = character(), 
                              "baseMean" = numeric(), 
                              "log2FoldChange" = numeric(), 
                              "lfcSE" = numeric(),
                              "stat" = numeric(),
...
#make master df with all conditions in it
for ( i in 1:length(conditions)) {
  print(i)
  res_name <- conditions[i]
  print(res_name)
  
  res <- results(dds, name = res_name)
  results_df <- res %>% as.data.frame() %>%
      rownames_to_column("gene_id") %>%
      merge(g2s) %>%
      mutate(result_name = res_name)
  
  res_shrunken <- lfcShrink(dds, coef = res_name,  res = res)
  res_shrunken_df <- res_shrunken %>% as.data.frame() %>%
  rownames_to_column("gene_id") %>%
  merge(g2s) %>%
  mutate(result_name = res_name)
  
  master_res_df <- bind_rows(master_res_df, results_df)
  master_res_shrunken_df <- bind_rows(master_res_shrunken_df, res_shrunken_df)
}
#check the dfs were merged correctly with each sample
table(master_res_df$result_name)
table(master_res_shrunken_df$result_name)

#convert into matrix with only the variables we need
...
dev.off()
##heatmap not plotting and I'm not sure why...
