...
```{r}
#making the data frame with all of the comparisons
##"total", "membrane_fraction", "insoluble_cytoplasmic_fraction", "cytosolic_fraction", "nuclear_fraction"
res <- results(dds, name = "condition_insoluble_cytoplasmic_fraction_vs_total")

res_shrunken <- lfcShrink(dds, coef = "condition_insoluble_cytoplasmic_fraction_vs_total",  res = res)

res_cic_df <- res %>% as.data.frame() %>%
  rownames_to_column("gene_id") %>%
  merge(g2s) %>%
...
res_shrunken <- lfcShrink(dds, coef = "condition_cytosolic_fraction_vs_total",  res = res)

res_cf_df <- res %>% as.data.frame() %>%
  rownames_to_column("gene_id") %>%
  merge(g2s) %>%
...
res_shrunken <- lfcShrink(dds, coef = "condition_nuclear_fraction_vs_total",  res = res)

res_nf_df <- res %>% as.data.frame() %>%
  rownames_to_column("gene_id") %>%
  merge(g2s) %>%
  mutate(result_name = "condition_nuclear_fraction_vs_total")

#merging the dfs
results_df <- rbind(res_cf_df,res_cic_df,res_nf_df,res_df)

#pivot_wider specify values and names to make the big matrix
lfc_matrix <- results_df %>% dplyr::select(gene_id, log2FoldChange, result_name) %>% pivot_wider(names_from = "result_name", values_from = "log2FoldChange")

lfc_matrix_na_omit_df <- na.omit(lfc_matrix)

#making the heatmap
lfc_matrix <- lfc_matrix_na_omit_df %>% column_to_rownames("gene_id") %>% as.matrix()

Heatmap(lfc_matrix, show_column_names = TRUE, show_row_names = FALSE) 
```
