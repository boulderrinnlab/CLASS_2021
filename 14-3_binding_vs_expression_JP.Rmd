...
write_csv(samplesheet, "/scratch/Shares/rinnclass/James/CLASS_2021/analysis/samplesheet_fixed_condition.csv")
```
...
```{r}
#Filter promoter_features_df to only give the 57 genes that are shared conservative resevoirs
...
```{r}
# Let's look at what's binding on conserved resevoirs and where it's binding.
AC011383.1_promoter <- all_promoters_gr[all_promoters_gr$gene_name == "AC011383.1"]
AC011383.1_promoter_matrix <- make_promoter_binding_matrix(consensus_peaks, AC011383.1_promoter)
plot_promoter_peak_matrix(AC011383.1_promoter_matrix, gene_name = "AC011383.1", save_pdf = TRUE)
```

```{r}
# Let's look at what's binding on conserved resevoirs and where it's binding.
AC104695.1_promoter <- all_promoters_gr[all_promoters_gr$gene_name == "AC104695.1"]
AC104695.1_promoter_matrix <- make_promoter_binding_matrix(consensus_peaks, AC104695.1_promoter)
plot_promoter_peak_matrix(AC104695.1_promoter_matrix, gene_name = "AC104695.1", save_pdf = TRUE)
```
