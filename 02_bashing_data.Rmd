...
```
awk '{if ($6 == "POLR2A") print $0;}' encode_awk_lessons.tsv > awk.txt
```
...
TODO debug
```
awk -F "/t" '{print $6}' encode_awk_lessons.tsv | wc -l
#this works but this does not
awk -F "/t" '{print $6}' encode_awk_lessons.tsv > dbp.tsv
```
alrigh we now see there are 500 or so lines in column 6 checking xcel that seems
right. 

Now we can print it to a new file 
...
Let's use sort or unique unix commands to determine the number of unique dbps

TODO STILL NO OUTPUT FILE ???
```
awk -F "/t" '{print $6}' encode_awk_lessons.tsv | sort -u > test.txt
...
What if you want everything except the POLR2A? we simply need to add the -v (for invert match)
If you want to grep through a folder you can use -r (grep -r 'term' folder/*)


Let's use awk, grep and pipes to make a new file from the .tsv

```
awk -F "/t" '{print $1 "/t" $2}' data/encode_awk_lessons.tsv > test.txt
```

```
awk ‘ {print $4} ‘ | awk ‘!($0 in array) { array[$0]; print} '
```
This puts the contents of column 4 “in” to a term called ‘array’ but is looking for all the contents in the array ($0) that are not in the array then prints unique values


```
 awk -F '/T' '{print $5}' data/encode_awk_lessons.tsv | sort -u data/encode_awk_lessons.tsv > test.txt
```
...
--------------------------------
for loops in BASH
# For / in / do / done will always be used


```
for file in *read*; do mv $file ${file%??}; done
```


# for is beginning to set up a term “file” 
# I was in an SFTP folder that I wanted to remove the extensions .1 and .2 from duplicate files
    ## so I did “
    
```
for *.gz; do mv $file ${file%??}; done
```

    
the do mv ${file%??} part is removing the last two characters (??) from the variable file
% is called a parameter extension which means remove text (could also use !)— the number of ?s is the number of characters removed from end of line (so 2 in this case)
...
#!/bin/bash
for i in $(awk -F "\t" '{print $19}' metadata.tsv | uniq); do
        echo $i
        awk -F "\t" '{OFS = ","}{print $1,$4,$19,$31,$37,$41,$43,$47}' metadata.tsv > selectcols.csv
        base_name='_filtered_meta.csv'
        head -n 1 selectcols.csv > "$i$base_name"
        grep $i selectcols.csv | grep 'fastq' >> "$i$base_name"
```




Let's use a for loop and radnom number generator to make the class groups:


beginer version (JR first for loop)
