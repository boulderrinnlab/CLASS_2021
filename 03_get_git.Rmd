...
Now let's got to your github website. On the top rigth ther is a drop down 
menu. Click on "settings". Now you will see on the left side there is a list of
options. Click on "SSH and CPG keys". There already be some keys in there or blank.
...
Do you see a new item (test.txt) that git noticed was made and ready to sync?
...
git status
git add test.txt
git commit -m (put the notes you want on what was done)
git push
# type git push again and you should see "up to date"

! ALWAYS REMEBER TO PUSH BEFORE YOU END A SESSION !
```
