...
column_to_rownames(peaks_per_dbp, var = "dbp") # same as bellow

peaks_per_dbp <- peaks_per_dbp %>%
  column_to_rownames(var = "dbp") # renames index as gene name

peaks_per_dbp["POLR2B",]
peaks_per_dbp["POLR2B", "total_peak_length"]
...
# before we did:
#peaks_per_dbp$id <-nrow(peaks_per_dbp)
#new way now:

peaks_per_dbp <- peaks_per_dbp %>%
  mutate(tpl_m = total_peak_length / 1000000)

# or defining the mutate function with dplyr
...
peaks_per_dbp <- peaks_per_dbp %>% 
  mutate(total_peak_length = total_peak_length / 1000000,
         total_peak_length = log2(total_peak_length))
head(peaks_per_dbp)
...
ifelse(peaks_per_dbp$tpl_m <50, "small", "large")

peaks_per_dbp <- peaks_per_dbp %>% 
  mutate(peak_genome_coverage = ifelse(tpl_m < 10, "small",
                          ifelse(tpl_m < 60, "medium", "large")))
