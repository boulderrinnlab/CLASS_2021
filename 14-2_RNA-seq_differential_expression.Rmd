...
<<<<<<< HEAD
res_shrunken_df_sig <- res_shrunken_df %>%
  filter(sig == "sig")

           
# results data.frame that contains all columns
# also a matrix


#to make matrix:
L2FC_matrix <- res_shrunken_df_sig %>% dplyr::select(gene_id, log2FoldChange, result_name) %>% pivot_wider(names_from = "result_name", values_from = "log2FoldChange") %>% column_to_rownames('gene_id') %>% as.matrix()


# clustering 
bin_hier <- hclust(dist(L2FC_matrix), method = "complete")
plot(bin_hier)

# heatmap
pheatmap(L2FC_matrix, show_rownames = F, breaks = seq(-3, 3, length.out = 100))

# Z-Scale the rows and Plot the dendrogram
mm_scaled <- t(scale(t(L2FC_matrix)))
