...
files <- file.path("/scratch/Shares/rinnclass/Michael/CLASS_2021/rnaseq/results/star_salmon",
...
# also a matrix
#to make matrix:
L2FC_matrix <- res_shrunken_df %>% dplyr::select(gene_id, log2FoldChange, result_name) %>% pivot_wider(names_from = "result_name", values_from = "log2FoldChange") %>% column_to_rownames('gene_id') %>% as.matrix()
